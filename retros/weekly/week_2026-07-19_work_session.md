...
- Status: Done — implemented 2026-07-19. Resolved as a heat-adjusted *pace* estimate (Mark Hadley temperature + dew point chart) rather than a bare flag: each activity stores dew point, apparent temperature, `heat_load_sum`, and `heat_pace_adjust_pct`, and the weekly log shows a heat-neutral equivalent pace when `sum >= 111`. Dew point (not humidity) drives the chart, added to the Open-Meteo fetch since only temperature was captured before. See `decisions/2026-07-19_heat_adjusted_pace_enrichment.md` and `docs/superpowers/specs/2026-07-19-heat-adjusted-pace-enrichment-design.md`. Follow-up left open: re-rendering the 8 legacy-format historical weekly logs (see decision record "Deferred").
